...
void solve(){
    rd(n)
    double mnSp_bnd = 1e18,mxSp_bnd = 0;    
    rep(i,1,n+1){
        rd(a)rd(b)
        double mnSp = (double)i/b,mxSp = (double)i/a; 
        mnSp_bnd = min(mnSp_bnd, mxSp);
        mxSp_bnd = max(mxSp_bnd, mnSp);
    }
    if(mxSp_bnd <= mnSp_bnd){
        cout<<fixed<<setprecision(6)<<mxSp_bnd; 
    } 
    else{
        cout<<-1;
    }
